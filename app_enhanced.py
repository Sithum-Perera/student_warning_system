...
# ================== NAVIGATION ==================
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Individual Assessment", "📁 Batch Analysis", "📈 Analytics Dashboard", "📋 Student Records", "⚙️ Settings"])
...
with tab1:
    ...
    with col2:
        ...
        district = st.text_input("District", "Kandy", help="Student's home district")
    ...
    def generate_enhanced_pdf(student_data, result, probability, recommendations):
        ...
        file_name = f"student_risk_report_{timestamp}.pdf"
        ...
        if errors:
            ...
            with col1:
                if st.button("📄 Generate Detailed Report", use_container_width=True):
                    pdf_file = generate_enhanced_pdf(log_data, result, probability, recommendations)
            ...
            with col2:
                if st.button("📧 Schedule Follow-up", use_container_width=True):
                    st.info("📅 Follow-up scheduled for next week. Notification sent to academic advisor.")
...
with tab2:
    ...
    with col1:
        uploaded_file = st.file_uploader(
            ...
            type=["csv"],
            help="CSV should contain columns: Gender, District, Attendance_Percentage, Study_Hours_per_Week, Previous_GPA, Internet_Access, Family_Income_LKR, Part_Time_Job, Sleep_Hours_per_Night"
        )
    ...
    if uploaded_file:
        try:
            ...
            required_cols = ['Gender', 'Attendance_Percentage', 'Study_Hours_per_Week', 'Previous_GPA', 'Internet_Access', 'Family_Income_LKR', 'Part_Time_Job', 'Sleep_Hours_per_Night']
...
with tab3:
    ...
    if os.path.exists("risk_log.csv"):
        try:
            log_df = pd.read_csv("risk_log.csv")
            ...
            if len(log_df) > 0:
                ...
                with col1:
                    # Risk by department
                    dept_risk = log_df.groupby('Department')['Risk'].apply(lambda x: (x == 'At-Risk').mean() * 100).reset_index()
                    dept_risk.columns = ['Department', 'At_Risk_Percentage']
                ...
                with col2:
                    # Monthly trend
                    if 'Month' in log_df.columns:
                        month_risk = log_df.groupby('Month')['Risk'].apply(lambda x: (x == 'At-Risk').mean() * 100).reset_index()
                        month_risk.columns = ['Month', 'At_Risk_Percentage']
            ...
            if len(log_df) > 0:
                ...
                with col2:
                    if st.button("🗑️ Clear All Records", use_container_width=True):
                        if st.checkbox("I confirm I want to delete all records"):
                            os.remove("risk_log.csv")
